<?php
namespace Gaobinzhan\Storage;

Use Gaobinzhan\Auth;
Use Gaobinzhan\Config;
Use Gaobinzhan\Zone;
Use Gaobinzhan\Http\Client;
Use Gaobinzhan\Http\Error;
...
*                                            ]
    ...
    public function listFiles(
        ...
        \Gaobinzhan\setWithoutEmpty($query, 'prefix', $prefix);
        \Gaobinzhan\setWithoutEmpty($query, 'marker', $marker);
        \Gaobinzhan\setWithoutEmpty($query, 'limit', $limit);
        \Gaobinzhan\setWithoutEmpty($query, 'delimiter', $delimiter);
...
*                                            ]
    ...
    public function listFilesv2(
        ...
        \Gaobinzhan\setWithoutEmpty($query, 'limit', $limit);
        \Gaobinzhan\setWithoutEmpty($query, 'delimiter', $delimiter);
        \Gaobinzhan\setWithoutEmpty($query, 'skipconfirm', $skipconfirm);
...
*                                              ]
    ...
    {
        $path = '/stat/' . \Gaobinzhan\entry($bucket, $key);
    ...
    {
        $path = '/delete/' . \Gaobinzhan\entry($bucket, $key);
    ...
    {
        $from = \Gaobinzhan\entry($from_bucket, $from_key);
        $to = \Gaobinzhan\entry($to_bucket, $to_key);
        $path = '/copy/' . $from . '/' . $to;
        if ($force === true) {
            $path .= '/force/true';
        }
    ...
    {
        $resource = \Gaobinzhan\entry($bucket, $key);
        $encode_mime = \Gaobinzhan\base64_urlSafeEncode($mime);
    ...
    public function fetch($url, $bucket, $key = null)
    {
        ...
        $resource = \Gaobinzhan\base64_urlSafeEncode($url);
        $to = \Gaobinzhan\entry($bucket, $key);
    ...
    {
        $entry = \Gaobinzhan\entry($bucket, $key);
    ...
    public static function buildBatchDeleteAfterDays($bucket, $key_day_pairs)
    {
        ...
        foreach ($key_day_pairs as $key => $day) {
            array_push($data, '/deleteAfterDays/' . \Gaobinzhan\entry($bucket, $key) . '/' . $day);
        }
        return $data;
    }
    ...
    public static function buildBatchChangeMime($bucket, $key_mime_pairs)
    {
        ...
        foreach ($key_mime_pairs as $key => $mime) {
            array_push($data, '/chgm/' . \Gaobinzhan\entry($bucket, $key) . '/mime/' . base64_encode($mime));
        }
        return $data;
    }
    ...
    public static function buildBatchChangeType($bucket, $key_type_pairs)
    {
        ...
        foreach ($key_type_pairs as $key => $type) {
            array_push($data, '/chtype/' . \Gaobinzhan\entry($bucket, $key) . '/type/' . $type);
        }
        return $data;
    }
    ...
    private static function oneKeyBatch($operation, $bucket, $keys)
    {
        $data = array();
        foreach ($keys as $key) {
            array_push($data, $operation . '/' . \Gaobinzhan\entry($bucket, $key));
        }
        return $data;
    }
    ...
    private static function twoKeyBatch($operation, $source_bucket, $key_pairs, $target_bucket, $force)
    {
        ...
        foreach ($key_pairs as $from_key => $to_key) {
            $from = \Gaobinzhan\entry($source_bucket, $from_key);
            $to = \Gaobinzhan\entry($target_bucket, $to_key);
