...
final class ResumeUploader
{
    ...
    public function __construct(
        ...
        list($accessKey, $bucket, $err) = \Gaobinzhan\explodeUpToken($upToken);
        if ($err != null) {
    ...
    public function upload($fname)
    {
        ...
        while ($uploaded < $this->size) {
            ...
            $crc = \Gaobinzhan\crc32_data($data);
            ...
            if ($response->statusCode < 0) {
                list($accessKey, $bucket, $err) = \Gaobinzhan\explodeUpToken($this->upToken);
                if ($err != null) {
                    return array(null, $err);
                }
    ...
    private function fileUrl($fname)
    {
        ...
        $url .= '/mimeType/' . \Gaobinzhan\base64_urlSafeEncode($this->mime);
        if ($this->key != null) {
            $url .= '/key/' . \Gaobinzhan\base64_urlSafeEncode($this->key);
        }
        $url .= '/fname/' . \Gaobinzhan\base64_urlSafeEncode($fname);
        if (!empty($this->params)) {
            foreach ($this->params as $key => $value) {
                $val = \Gaobinzhan\base64_urlSafeEncode($value);
                $url .= "/$key/$val";
            }
        }
        return $url;
