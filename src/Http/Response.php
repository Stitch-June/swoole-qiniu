...
final class Response
{
     ...
     */
    public function __construct(\Gaobinzhan\Coroutine\Http\Response $response)
    {
        $code = $this->statusCode = $response->getStatusCode();
        $duration = $this->duration = 0;
        $headers = $this->headers = $response->getHeaders();
        $body = $this->body = $response->getBody();
        $error = $this->error = $response->getErrorMsg();
        $jsonData = $this->jsonData = $response->getJsonData();
        if ($error !== null) {
            return;
        }
    ...
    {
        return \Gaobinzhan\json_decode((string)$body, true, 512);
    }
